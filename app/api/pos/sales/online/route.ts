...
class PosOnlineSaleValidationError extends Error {}
...
export async function POST(req: NextRequest) {
  ...
  const rateLimit = checkRateLimit(req, { key: "pos-online-sale", limit: 20, windowMs: 10 * 60_000 })
  if (!rateLimit.allowed) {
    return NextResponse.json({ error: "Too many online payment attempts. Wait a few minutes and try again." }, { status: 429, headers: rateLimitHeaders(rateLimit.retryAfterSeconds) })
  }
  ...
  try {
    const sale = await prisma.$transaction(async (tx) => {
      ...
      for (const item of saleItems) {
        ...
        if (product.price <= 0) {
          throw new PosOnlineSaleValidationError(`${product.name} needs a price before it can be sold`)
        }
        if (product.quantity < item.quantity) {
          throw new PosOnlineSaleValidationError(`Only ${product.quantity} ${product.name} available`)
        }
        ...
        if (updated.count !== 1) throw new PosOnlineSaleValidationError(`${product.name} changed while starting payment`)
