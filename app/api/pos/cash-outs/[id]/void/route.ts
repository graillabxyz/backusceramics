...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!operator) return NextResponse.json({ error: "Unlock the POS before voiding cash activity.", code: "POS_PIN_LOCKED" }, { status: 423 })
  ...
  const updated = await prisma.posCashOut.updateMany({
    where: { id, voidedAt: null, reimbursedAt: null },
    data: { voidedAt: new Date(), voidedById: operator.id, voidReason: reason },
  })
  if (updated.count !== 1) return NextResponse.json({ error: "This entry was already reimbursed, voided, or no longer exists." }, { status: 409 })
